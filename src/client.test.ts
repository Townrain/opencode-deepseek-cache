...
describe('pickPrimaryBalance', () => {
  ...
  it('returns the single item for array of one', () => {
    const info: BalanceInfo = { currency: 'USD', total_balance: '10.00' }
    expect(pickPrimaryBalance([info])).toBe(info)
  })

  it('picks the balance with largest total_balance', () => {
    const low: BalanceInfo = { currency: 'USD', total_balance: '5.00' }
    const high: BalanceInfo = { currency: 'USD', total_balance: '42.50' }
    const mid: BalanceInfo = { currency: 'USD', total_balance: '20.00' }
    expect(pickPrimaryBalance([low, high, mid])).toBe(high)
  })

  it('handles string comparison correctly via Number conversion', () => {
    const a: BalanceInfo = { currency: 'USD', total_balance: '9.99' }
    const b: BalanceInfo = { currency: 'USD', total_balance: '100.00' }
    expect(pickPrimaryBalance([a, b])).toBe(b)
  })

  it('returns first item when all are equal', () => {
    const a: BalanceInfo = { currency: 'USD', total_balance: '10.00' }
    const b: BalanceInfo = { currency: 'CNY', total_balance: '10.00' }
...
describe('getBalance', () => {
  ...
  it('uses custom baseUrl', async () => {
    ...
    expect(globalThis.fetch).toHaveBeenCalledWith(
      'https://custom.api.com/user/balance',
      expect.anything(),
    )
  })
